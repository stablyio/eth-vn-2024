...
contract QuadraticBorrowCompound is IQuadraticBorrowCompoundStorage,IQuadraticLendCompoundStorage,RoleControl,Initializable {
    ...
    function doInitialize(address _uniswapV3,address _uniswapV3Oracle, IQuadraticLendCompound _iQuadraticLendCompound,address _quadraticAuction,uint256 _globalDefault,uint256 _interestPlatformRate) public initializer{
    ...
    function transferToAuction(uint256 bid) external nonReentrant whenNotPaused{
        ...
        IQuadraticAuction(quadraticAuction).toAuction(address(uniswapV3),_borrowInfo.tokenId,bid,_borrowPool.token,_borrowInfo.auctionValue,_borrowInfo.interests);
        
        uniswapV3.transferFrom(address(this), quadraticAuction, _borrowInfo.tokenId);
    ...
    function _updateRealReturnInterest(uint256 _pid,uint256 _interests) private {
        if(_interests > 0){
            CompoundBorrowPool storage _borrowPool = borrowPoolInfo[_pid];
            uint256 lpSupply = QuadraticFinanceToken(_borrowPool.ctoken).totalSupply();
            if (lpSupply > 0) {
